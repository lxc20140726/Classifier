...
输出端口:
  items  PROCESSING_ITEM_LIST  "Files 字段更新为打包后文件列表"
...
| 14 | `compress-node` | action | `items: PROCESSING_ITEM_LIST` | `items: PROCESSING_ITEM_LIST` | 写归档文件 | 删除归档/恢复原文件 |
...
compress-node 输出端口（修正后）:
  items           PROCESSING_ITEM_LIST   "Files 字段更新为打包后文件列表"
  archive_paths   STRING_LIST            "生成的归档文件绝对路径列表（供 Rollback 使用）"
```
