...
func (s *WorkflowRunnerService) ApproveProcessingReview(ctx context.Context, workflowRunID, reviewID string) error {
	...
	if err := s.reviews.UpdateDecision(ctx, review.ID, "approved", "", &now); err != nil {
		return fmt.Errorf("workflowRunner.ApproveProcessingReview update review %q: %w", review.ID, err)
	}
	if strings.TrimSpace(review.FolderID) != "" {
		if err := s.folders.UpdateStatus(ctx, review.FolderID, "done"); err != nil {
			return fmt.Errorf("workflowRunner.ApproveProcessingReview update folder %q status: %w", review.FolderID, err)
		}
	}
	if err := s.writeProcessingReviewAudit(ctx, run, review, "workflow.processing.review_approved", "success", nil); err != nil {
		return fmt.Errorf("workflowRunner.ApproveProcessingReview write audit: %w", err)
	}
...
func (s *WorkflowRunnerService) RollbackProcessingReview(ctx context.Context, workflowRunID, reviewID string) error {
	...
	if err := s.rollbackWorkflowRunForReview(ctx, run, review); err != nil {
		return fmt.Errorf("workflowRunner.RollbackProcessingReview rollback review %q: %w", reviewID, err)
	}

	now := time.Now()
	if err := s.reviews.UpdateDecision(ctx, review.ID, "rolled_back", "", &now); err != nil {
		return fmt.Errorf("workflowRunner.RollbackProcessingReview update review %q: %w", review.ID, err)
	}
	if strings.TrimSpace(review.FolderID) != "" {
		if err := s.folders.UpdateStatus(ctx, review.FolderID, "pending"); err != nil {
			return fmt.Errorf("workflowRunner.RollbackProcessingReview update folder %q status: %w", review.FolderID, err)
		}
	}
	if err := s.writeProcessingReviewAudit(ctx, run, review, "workflow.processing.review_rolled_back", "success", nil); err != nil {
		return fmt.Errorf("workflowRunner.RollbackProcessingReview write audit: %w", err)
	}
...
func buildProcessingReviewDiff(before map[string]any, after map[string]any, stepResults []ProcessingStepResult) map[string]any {
	...
	return map[string]any{
		"path_changed":   beforePath != "" && afterPath != "" && beforePath != afterPath,
...
func summarizeSteps(results []ProcessingStepResult) []map[string]string {
	...
	for _, step := range results {
		...
		key := strings.Join([]string{nodeType, strings.TrimSpace(step.NodeLabel), strings.TrimSpace(step.Status)}, "|")
