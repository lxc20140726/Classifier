...
func (h *FolderHandler) GetClassificationTree(c *gin.Context) {
	...
	if err := h.populateTreeFiles(c, &tree); err != nil {
		return
	}
...
func (h *FolderHandler) populateTreeFiles(c *gin.Context, node *folderClassificationTreeEntry) error {
	...
	for i := range node.Subtree {
		if err := h.populateTreeFiles(c, &node.Subtree[i]); err != nil {
			return err
		}
	}
